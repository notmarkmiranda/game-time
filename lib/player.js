...
Player.prototype.move = function(map){
  ...
  // var newX = this.calcMovement(this.x, Math.cos);
  // var newY = this.calcMovement(this.y, Math.sin);
...
Player.prototype.calcMovement = function(position, calc){
  var moveStep = this.speed * this.moveSpeed;
  this.rotation += this.direction * this.rotSpeed * Math.PI / 180;
  return position + calc(this.rotation) * moveStep;
}
